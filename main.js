...
/* =====================================================================
 * § 1A  UFO LIGHT RAYS (ReactBits LightRays → Vanilla JS)
 * =====================================================================
 *
 *  WebGL volumetric beam effect using ogl.
 *  Renders a full-screen fragment shader pass inside the beam container.
 *  Controlled by scroll progress via setOpacity / setIntensity.
 *
 *  Dependency: ogl (loaded dynamically via ES module import)
 * ===================================================================== */
...
class UfoLightRays {
  ...
  _init(Renderer, Program, Triangle, Mesh) {
...
float beamStrength(vec2 source, vec2 dir, vec2 coord, float speed) {
  vec2 toCoord = coord - source;
  float dist = length(toCoord);
  vec2 dirNorm = normalize(toCoord);

  // Wide enough to attach to UFO, narrow enough to taper
  float discRadius = iResolution.x * 0.055;
  float discBlend = smoothstep(0.0, discRadius, dist);
  float cosAngle = mix(1.0, dot(dirNorm, dir), discBlend);

  // STABLE cone — distortion does not touch this
  float spread = pow(max(cosAngle, 0.0), 1.0 / max(lightSpread, 0.001));

  // Soft bottom fade
  float maxDist = iResolution.x * rayLength;
  float lengthFalloff = 1.0 - smoothstep(maxDist * 0.55, maxDist, dist);

  // Downward energy flow
  float flowSpeed = speed * 4.0;
  float flow = noise(vec2(dist * 0.018 - iTime * flowSpeed, dirNorm.x * 2.5));

  // Fine texture — distortion only in phase, not cone
  float phaseShift = distortion * sin(iTime * 4.0 + dist * 0.04) * 0.15;
  float angleVar = cosAngle * 26.0 + iTime * speed * 2.0 + phaseShift;
  float texture = 0.78 + 0.08 * sin(angleVar) + 0.06 * flow;

  return texture * lengthFalloff * spread;
}

void main() {
  vec2 coord = vec2(gl_FragCoord.x, iResolution.y - gl_FragCoord.y);

  // Single cohesive beam layer (no second interfering ray system)
  float beam = beamStrength(rayPos, rayDir, coord, raysSpeed);

  vec4 fragColor = vec4(vec3(beam), beam);

  if (noiseAmount > 0.0) {
    float n = noise(coord * 0.012 + iTime * 0.12) * 0.2;
    fragColor.rgb *= (1.0 - noiseAmount + noiseAmount * n);
  }

  fragColor *= 1.5;
  fragColor.rgb *= raysColor * globalIntensity;
  ...
  float alpha = max(max(fragColor.r, fragColor.g), fragColor.b) * globalOpacity;
  ...
  _updatePlacement() {
    ...
    // Origin: top-center of container (UFO underside), pointing down
    this.uniforms.rayPos.value = [w * 0.5, 0];
    this.uniforms.rayDir.value = [0, 1];
  }
  ...
  _startLoop() {
    const loop = (t) => {
      ...
      try {
        ...
        console.warn('UfoLightRays render error:', err);
      }
    };
  ...
  /** 0.001 to 1.0+ — how wide the beam spreads */
  setSpread(val) {
    if (this.uniforms) {
      this.uniforms.lightSpread.value = Math.max(0.001, val);
    }
  }

  /** 0.0 to 1.0+ — how far the rays reach */
  setLength(val) {
    if (this.uniforms) {
      this.uniforms.rayLength.value = Math.max(0, val);
    }
  }

  /** 0.0 to 1.0+ — animation speed multiplier */
  setAnimationSpeed(val) {
    if (this.uniforms) {
      this.uniforms.raysSpeed.value = val;
    }
  }

  /** Update color dynamically (hex string) */
  setColor(hex) {
    if (this.uniforms) {
      this.uniforms.raysColor.value = this._hexToRgb(hex);
    }
  }
  ...
  /** Clean up WebGL context and remove canvas */
  destroy() {
...
/* =====================================================================
 * § 1B  GALAXY STARFIELD (ReactBits → Vanilla)
 *  Replaces the 2D #stars canvas in dark mode only.
 *  Lives entirely inside .hero-pin. Does not touch body-environment.
 * ===================================================================== */
...
class GalaxyBackground {
  constructor(options, ogl) {
    ...
    this.mouseInteraction = options.mouseInteraction ?? true;
    this.glowIntensity = options.glowIntensity ?? 0.3;
    this.saturation = options.saturation ?? 0.0;
    this.mouseRepulsion = options.mouseRepulsion ?? true;
    ...
    this._init(Renderer, Program, Triangle, Mesh, Color);
  }

  _init(Renderer, Program, Triangle, Mesh, Color) {
    const renderer = new Renderer({
...
`;
    ...
    const update = (t) => {
      ...
      program.uniforms.uMouseActiveFactor.value = this.smoothMouseActive;
...
function initGalaxy() {
  ...
  import('https://esm.sh/ogl@1.0.8')
    ...
    .catch((err) => {
      console.warn('Galaxy: failed to load ogl', err);
    });
}
...
/* =====================================================================
 * § 6A  UFO LIGHT RAYS INITIALIZATION
 * ===================================================================== */
...
/*
 * Animate CONTENT rather than .card-feat itself.
 *
 * .card-feat's transform belongs to the existing
 * desktop / phone holo tilt systems.
 */
const previousContent = [
  previousFull.querySelector('.card-vis'),
  previousFull.querySelector('.card-body')
].filter(Boolean);

const nextContent = [
  nextFull.querySelector('.card-vis'),
  nextFull.querySelector('.card-body')
].filter(Boolean);
 ...
 * PHASE 1 — HANDOFF
 *
 * The outgoing card softens first.
 *
 * At the same time, the selected compact row gets
 * a very small "energy" response before opening.
 * ---------------------------------------------------------------
 */
...
accordionTween =
  gsap.timeline({
    ...
    onComplete: () => {
      ...
      /*
       * Perform the actual accordion state swap.
       */
      activeIndex =
        nextIndex;
      ...
      /*
       * Remove the temporary outgoing presentation
       * styles now that those pieces are hidden.
       */
      gsap.set(
        previousContent,
        {
      ...
      /*
       * Allow both wrappers to reveal their new
       * natural accordion heights.
       */
      gsap.set(
        [
          previousCard,
      ...
      /*
       * Restore the sizes the user was looking at
      ...
      /*
       * Incoming full-card content starts slightly
       * below its final resting position.
       *
       * We animate the card's CONTENT — not the
       * tilt-enabled .card-feat wrapper.
       */
      gsap.set(
        nextContent,
        {
      ...
      /*
       * The collapsed replacement row also comes
       * in gently instead of simply appearing.
       */
      gsap.set(
        previousMini,
        {
       ...
       * PHASE 2 — MORPH
       *
       * Collapse gets out of the way first.
       * Expansion follows just behind it and receives
       * the softer, longer landing.
       * -----------------------------------------------------------
       */
      ...
      /*
       * Old full card contracts first.
       *
       * Slightly quicker than the expansion so it
       * feels like it's yielding space.
       */
      accordionTween.to(
        previousCard,
        {
      ...
      /*
       * New card begins expanding shortly AFTER
       * the collapse begins rather than moving in
      ...
      /*
       * The outgoing card's new compact selector
       * settles into the vacated space.
       */
      accordionTween.to(
        previousMini,
        {
      ...
      /*
       * Don't immediately reveal the entire incoming
       * card. Let the container open first.
       */
      accordionTween.to(
        nextContent,
        {
...
/*
 * Outgoing visual/body soften independently.
 *
 * Very small movement — just enough to signal that
 * this card is releasing focus.
 */
accordionTween.to(
  previousContent,
  {
...
/*
 * Tiny "charge" through the selected row's existing
 * accent pieces. No new colors or visual tokens.
 */
accordionTween.to(
  [
    nextNumber,
...
/*
 * Selected mini row fades only after the accent
 * response has begun.
 */
accordionTween.to(
  nextMini,
